...
test("personal center uses preferences and gear only to preset unassigned items", async () => {
  ...
  assert.match(page, /运动 \/ 健身/);
  assert.match(page, /户外 \/ 徒步 \/ 露营/);
  assert.match(page, /label: "观星"/);
  assert.match(page, /带娃出行/);
  assert.match(page, /宠物出行/);
  assert.match(page, /钓鱼/);
  assert.match(page, /滑雪 \/ 冰雪活动/);
  assert.match(page, /水上运动 \/ 潜水/);
  assert.match(page, /温泉 \/ 泡汤/);
  ...
  assert.match(page, /items: \["相机", "备用电池", "内存卡", "充电宝", "三脚架"\]/);
  assert.match(page, /items: \["运动服", "运动鞋", "水壶", "护膝", "运动相机"\]/);
  assert.match(page, /items: \["登山杖", "急救包", "防风外套", "头灯", "防潮垫"\]/);
  assert.match(page, /items: \["儿童证件", "儿童常用药", "湿巾", "备用衣物", "玩具"\]/);
  assert.match(page, /items: \["宠物证件", "宠物粮", "水碗", "牵引绳", "宠物常用药"\]/);
  assert.match(page, /items: \["钓具", "鱼线 \/ 鱼钩", "防晒帽", "折叠椅", "防水袋"\]/);
  assert.match(page, /items: \["滑雪服", "护膝 \/ 护腕", "手套", "暖宝宝"\]/);
  assert.match(page, /items: \["泳衣 \/ 潜水服", "防水袋", "防晒霜", "拖鞋", "毛巾"\]/);
  assert.match(page, /items: \["泳衣 \/ 浴衣", "拖鞋", "毛巾", "防水袋", "补水喷雾"\]/);
  assert.doesNotMatch(page, /label: "(?:美食|手作 \/ 绘画|商务出行|音乐 \/ 演出|骑行|自驾游)"/);
