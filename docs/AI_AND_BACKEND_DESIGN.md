...
客户端修改后 450ms 合并保存，服务端使用 `expectedVersion` 乐观锁，并以 D1 作为唯一权威数据源：
...
- 境外目的地将交通卡、流量卡/eSIM 提升为高优先级候选；境内目的地在提示词和服务端结果校验两层排除流量卡、境外交通卡和转换插头，避免把境外建议带入成都等国内行程。
...
- 请求中：显示“正在检查清单”。
- 1–2 条：卡片可加入、移出和删除。
- 0 条：保留紧凑完成态“AI 检查完成，当前清单已覆盖常见遗漏”，避免用户把模块消失误解为请求失败。
...
- `claim`：被指派成员明确说“我来带 / 我带”，主动认领物品。
- `release`：被指派成员明确反悔、不再携带。
- `request`：有人提出携带请求，或对方只回复“好 / 行 / 可以”；此时仍需通过确认卡决定是否写入清单。
...
- `request` 不会直接替别人认领；“好 / 行 / 可以”只弹出确认卡，点击“我来带”才写入清单。
...
| POST | `/api/weather` | 点击“生成清单”后在后台查询天气；结果仅作为 AI 补漏上下文，不在首页展示 |
...
- 天气服务失败：后台静默降级为目的地 + 月份 + 当地季节，不阻塞生成清单，也不向用户暴露技术状态。
