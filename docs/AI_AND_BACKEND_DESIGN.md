...
| 2 条建议 | 两张并排卡片 |
| 1 条建议 | 单张卡片，不用空卡占位 |
| 用户加入 1 条 | 该卡按钮变为“移出清单”，保留另一条建议 |
| 用户移出 1 条 | 从统一物品数据删除对应条目，卡片恢复“加入清单”；支持 3 秒撤回 |
| 两条均已加入 | 两张卡片保留为可逆状态，用户可直接移出任意一条 |
| 0 条可推荐 | 展示“当前清单已覆盖常见易漏物品”而不是空白 |
| 请求失败 | 使用目的地回退建议；仍失败则显示轻量重试入口 |

推荐卡片保留的原因是让 AI 决策可回退：用户误加或临时改变计划时，不必进入编辑模式寻找并删除。若后续需要进一步压缩页面，可在两条均加入后收起为一行，并提供“查看已加入建议”的恢复入口。
...
- 出发日期、天数、季节和天气；
- 城市 / 海边 / 雪地 / 演唱会等活动类型；
- 用户设备档案，例如是否有相机或大容量充电宝；它只用于预设相关物品，不直接参与分工；
- 航班、托运和住宿条件；
- 用户明确的拍照、护肤、亲子或运动偏好。
