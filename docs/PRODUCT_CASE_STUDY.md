...
最初的灵感来自朋友共同编辑的旅行物品表格。结伴出行并不是几个人分别维护互不相关的个人清单：洗面奶、防晒、雨伞和充电宝等物品是否需要各自携带，往往取决于队友当前的准备选择。

这种相互依赖容易产生三类信息差：A 与 B 都以为对方会带，最后无人携带；两个人没有看到彼此决定，造成无意识重复；一方看到队友认领后决定不带，但对方后来改变计划却没有及时同步。即使聊天中已经说好，口头承诺也可能被后续消息淹没；完成认领后，拖延和遗忘仍可能导致物品没有真正装箱。

因此，“收拾行李”在多人场景中是一个从团队决策到个人执行的协作任务。产品需要持续回答三个问题：团队需要什么、当前谁决定携带、出发前本人是否已经装好。

### 用户的真实任务

用户并不是想“管理一张表”，而是连续完成四件事：

1. 看见队友对每件团队物品作出的携带选择。
2. 据此决定自己认领、与队友同时携带，或暂时不带。
3. 把聊天中的请求、同意和反悔更新为所有成员可见的最新状态。
4. 临出发时只核对自己真正需要装包的物品。
...
因此，产品机会不是做一张更漂亮的表格，也不是单纯生成更多物品，而是把自然交流与结构化分工连接起来，让物品、携带成员、变更记录和个人核对共享同一数据源。
...
- 识别分工时，AI 先生成确认卡，由对应成员本人确认后才修改清单；任何成员也可以直接手动认领。
...
### AI 功能二：从目的地推断“当前清单缺口”

它是协作主流程之外的辅助能力。系统同时把已有清单传给模型，让 AI 做差异补充，而不是重新生成一份长清单；即使模型失败，团队认领、聊天和核对仍可正常使用。

两项 AI 能力都遵循同一原则：**模型提出候选，对应成员本人确认，结构化数据落库。**
...
### 亮点 3：成员本人确认，避免 AI 替用户承诺

多人清单属于共享事实，错误修改会影响所有人。因此他人指派、模糊表达和模型推断都不直接落库，而是先征求对应成员本人确认；任何成员仍可直接手动认领自己的任务。
...
- 认领、删除、排序、留言、聊天和核对均由服务端保存；同一运行实例内通过 WebSocket 立即通知客户端刷新，客户端指数退避重连，2.5 秒版本轮询负责断线与跨实例兜底。
