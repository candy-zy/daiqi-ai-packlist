...
规则引擎先根据**目的地、用户偏好和已有设备**预设清单：例如摄影会补相机、备用电池、内存卡、手机稳定器与三脚架，户外和观星也有各自明确的物品映射；这些物品只进入清单，不会替用户认领。随后 AI 读取目的地、预设后的已有物品与出行偏好，只返回仍未覆盖、且容易忽略的 0–2 件物品。结构化输出、同义词去重与失败回退保证结果能直接进入产品流程；没有合适建议时，卡片会自动消失。
