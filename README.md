...
AI 同时读取**目的地、已有物品、用户偏好和设备拥有情况**，只返回清单中没有、且容易忽略的 0–2 件物品。结构化输出、同义词去重与失败回退保证结果能直接进入产品流程；没有合适建议时，卡片会自动消失。

<table>
  <tr>
    <td align="center" width="50%"><img src="docs/assets/demo-02-personalization.jpg" width="340" alt="个人中心与旅行偏好"/><br/><sub>偏好决定个人提醒；设备只用于推荐分工，不替用户认领</sub></td>
...
|---|---|---|---|---|
| 目的地补漏 | 目的地、天数、已有清单、偏好、设备 | 0–2 个物品 + 一句话理由 | 结构化 Schema、同义词去重、最多 2 条、失败回退 | 建议采纳率、重复率、删除率、响应时延 |
...
| 个性化清单策略 | ✅ 已实现 | 偏好影响个人提醒，设备用于团队分工建议 |
